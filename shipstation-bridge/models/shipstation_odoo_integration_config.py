...
class ShipstationOdooIntegationConfig(models.Model):
    _name = "shipstation.odoo.configuration"
    ...
    def action_get_shipstation_stores(self):
        action = self.env.ref('shipstation-bridge.actionid_shipstation_store').read()[0]
        return action

    def action_action_get_shipstation_providers(self):
        action = self.env.ref('shipstation-bridge.actionid_shipstation_delivery_carrier').read()[0]
        return action

    def action_get_shipstation_services(self):
        action = \
        self.env.ref('shipstation-bridge.actionid_shipstation_delivery_carrier_service').read()[0]
        return action

    def action_get_shipstation_packages(self):
        action = self.env.ref('shipstation-bridge.actionid_shipstation_delivery_package').read()[0]
        return action
